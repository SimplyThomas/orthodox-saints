...
**Goal:** Turn a generated batch into a single reviewer-facing view that shows, per profile, the prose, the verifier's flags, the coverage verdict, and per-claim sources — so reviewing draft profiles is low-friction (and the underlying YAML is itself readable), without a live page (which hides exactly that safety information).
...
- `src/content/profiles/OS-####.yaml` — draft profiles (Plan 1 YAML format).
...
| `tools/profilegen/profile_read.py` (**new**) | Read a profile `.yaml` file into a dict (`yaml.safe_load`; authoring-only `pyyaml`). |
...
## Task 1: Read a profile YAML file to a dict

**Files:** Create `tools/profilegen/profile_read.py`; Test: `tests/test_review_sheet.py`

YAML parses directly — no fragile string-slicing (a clear win of the YAML storage switch).
Uses authoring-only `pyyaml` (`pip install pyyaml`), like the rest of the pipeline tooling.
...
```python
"""Read a per-saint profile YAML file to a dict (authoring-only; needs pyyaml)."""
from pathlib import Path
...
- [ ] **Step 5: Verify against a real profile file** (after Plan 1 exists)

Run: `python -c "from pathlib import Path; from tools.profilegen import profile_read; print(profile_read.read(Path('src/content/profiles/OS-0021.yaml'))['id'])"`
Expected: `OS-0021`.
...
git commit -m "feat(review): read a profile YAML file to a dict"
```
...
class LoadBatchTests(unittest.TestCase):
    def test_joins_by_id(self):
        ...
        profiles = d / "profiles"
        profiles.mkdir()
        (profiles / "OS-0042.yaml").write_text(yaml.safe_dump(
            {"id": "OS-0042", "overview": ["A life."],
...
def load_batch(coverage_csv: Path, verdicts_json: Path, profiles_dir: Path) -> list[dict]:
    ...
    for sid, cov in coverage.items():
        prof_path = profiles_dir / f"{sid}.yaml"
...
import yaml as _y; (pr/"OS-0042.yaml").write_text(_y.safe_dump({"id":"OS-0042","overview":["A life."],"status":"draft","sources":["https://oca.org/x"]},sort_keys=False))
out=d/"review.html"
...
profile-review: ## render a batch review sheet (DATE=YYYY-MM-DD of the batch run)
	python -m tools.profilegen.review_sheet \
	  ...
	  src/content/profiles dist/profile_review_$(DATE).html
```
