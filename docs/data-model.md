...
| `saint_hymns.csv` | `saint_id,kind,tone,text,translation,source_url` | hymn (many per saint) | The troparion/kontakion **itself** (column 18 stays a derived *search link*). `kind` ∈ `Troparion`/`Kontakion`/`Apolytikion`/`Megalynarion`/`Ikos`/`Exapostilarion`/`Sticheron`; `tone` = `1`–`8`, `Plagal of the First`…`Fourth`, or `Grave`. Stanzas split on **`\|\|`**, not `"; "` — hymns are full of semicolons. `translation` is the §9 gate: a **public-domain** source (as for quotes) **or** a `Permission:<source_slug>` token; a permission row also **requires** `source_url`. |
...
| `groups.csv` | `slug,saint_id,name,type,description,feast,sort,rule` | group | `slug` is permanent. `saint_id` is the group's own **build-assigned OS-#### (blank → assigned; §6 shared counter with saints)** — the group is a saint-profile served at `/saint/<saint_id>` with `profile_type:"group"`. `type` ∈ `synaxis`/`feast-companions`/`household`. **`rule`** (optional) makes an **open/dynamic** group: `build.py` computes its members by matching saints instead of using explicit `saint_groups.csv` rows, so a newly-glorified saint joins automatically. Grammar: ` && `-joined conditions `field:v1\|v2` (AND across conditions, OR within; case-insensitive substring); fields = rank, era, century, region, tradition, vocation, life, notes, name. E.g. New Martyrs of Russia = `rank:New Martyr\|Confessor\|Passion-bearer && era:Modern && tradition:Russian`. |
...
| `image_permissions.csv` | `vendor_slug,vendor_name,attribution,homepage,granted,status,terms` | vendor grant | Registry for `Permission:<slug>` **image** licenses; grants recorded under `docs/permissions/`. |
| `text_permissions.csv` | `source_slug,source_name,attribution,homepage,granted,status,terms` | text grant | The sibling registry, for `Permission:<slug>` **text** tokens (`saint_hymns.csv`). Kept separate on purpose: the OCA granted its texts and stated it cannot grant its icons, so a text grant must never satisfy the image gate. `status=revoked` stops every text from that source shipping. |
