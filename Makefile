.PHONY: build validate test serve xlsx find report profile-batch profile-coverage profile-run profile-stop profile-status \
        feast-batch feast-run feast-stop feast-status download-icons icon-sheet clean \
...
feast-batch:  ; python -m tools.feastgen.prioritize $(or $(N),10)   # print N high-value profile-less feast IDs for a generation batch (N=10 default)
feast-run:    ; python -m tools.feastgen.run   # hands-off runner for feast profiles (headless claude -p loop; resumable)
feast-stop:   ; @if [ -f dist/feastgen/run.pid ]; then kill -TERM "$$(cat dist/feastgen/run.pid)" && echo "Sent SIGTERM to PID $$(cat dist/feastgen/run.pid) — run will stop after the current batch"; else echo "feastgen is not running (no dist/feastgen/run.pid)"; fi
feast-status: ; @if [ -f dist/feastgen/state.json ]; then cat dist/feastgen/state.json; else echo "No state file found (run has not started or dist/ was cleaned)"; fi
